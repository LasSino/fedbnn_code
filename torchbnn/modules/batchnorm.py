    ...
    def __init__(self, prior_mu, prior_sigma, num_features, eps=1e-5, momentum=0.1, affine=True, track_running_stats=True):
        ...
        if self.affine:
            ...
            self.register_buffer("prior_weight_mu", 
                    torch.Tensor(num_features))
            self.register_buffer("prior_weight_log_sigma", 
                    torch.Tensor(num_features))
            self.prior_weight_mu.fill_(self.prior_mu)
            self.prior_weight_log_sigma.fill_(self.prior_log_sigma)
            
            self.weight_mu = Parameter(torch.Tensor(num_features))
            self.weight_log_sigma = Parameter(torch.Tensor(num_features))
            self.register_buffer('weight_eps', None)
            
            self.register_buffer("prior_bias_mu", torch.Tensor(num_features))
            self.register_buffer("prior_bias_log_sigma",
                    torch.Tensor(num_features))
            self.prior_bias_mu.fill_(self.prior_mu)
            self.prior_bias_log_sigma.fill_(self.prior_log_sigma)
