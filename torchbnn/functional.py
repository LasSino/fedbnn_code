...
def _kl_loss(mu_0, log_sigma_0, mu_1, log_sigma_1) :
    ...
    (auto_exp(log_sigma_0)**2 + (mu_0-mu_1)**2)/(2*auto_exp(log_sigma_1)**2) - 0.5
    return kl.sum()
...
def bayesian_kl_loss(model, reduction='mean', last_layer_only=False) :
    ...
    for m in model.modules() :
        if isinstance(m, (BayesLinear, BayesConv2d)):
            kl = _kl_loss(m.weight_mu, m.weight_log_sigma, m.prior_weight_mu, m.prior_weight_log_sigma)
            kl_sum += kl
            n += len(m.weight_mu.view(-1))

            if m.bias :
                kl = _kl_loss(m.bias_mu, m.bias_log_sigma, m.prior_bias_mu, m.prior_bias_log_sigma)
                kl_sum += kl
